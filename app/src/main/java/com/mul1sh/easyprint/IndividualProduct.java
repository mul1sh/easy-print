...
public class IndividualProduct extends AppCompatActivity {
    ...
    private void initialize() {
        ...
        if (model != null) {
            productprice.setText(String.format("$ %s", Float.toString(model.getCardprice())));
        }
    ...
    public void goToCart(View view) {
        ...
        if ( customheader.getText().toString().length() == 0 ||  custommessage.getText().toString().length() ==0 ){
            ...
            SingleProductModel myModel = getProductObject();
            mDatabaseReference.child("cart").child(usermobile).push().setValue(myModel);
            session.increaseCartValue();
